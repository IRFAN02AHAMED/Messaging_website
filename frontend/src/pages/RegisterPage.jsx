...
const PHONE_REGEX = /^(\d{10}|\+\d{10,15})$/;
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!PHONE_REGEX.test(trimmedPhone)) {
      setError('Phone number must be a 10-digit number or in E.164 format (e.g., +1234567890)');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Registration failed. Phone number might already be registered.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          >
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
              Register a new account to start messaging
            </Typography>
          </Box>

          <Divider />

          <Box sx={{ p: 4 }} component="form" onSubmit={handleRegister}>
            {error && (
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <PersonIcon sx={{ color: '#8696A0', fontSize: 20 }} />
                  </InputAdornment>
                ),
              }}
            />
            ...
            <Button
              ...
              fullWidth
              variant="contained"
              disabled={loading || success || !name.trim() || !phone.trim() || !password}
              sx={{
                py: 1.5,
                fontSize: '1rem',
                fontWeight: 600,
